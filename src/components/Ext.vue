...
// 设置插件的激活状态
function setEnable() {
  browser.management.setEnabled(props.extInfo.id, !props.extInfo.enabled)
    .catch((err) => {
...
function uninstallExt() {
  ...
  browser.management.uninstall(props.extInfo.id)
}

// 打开插件的option页面
function openExtOption() {
  if (props.extInfo.optionsUrl !== '')
    browser.tabs.create({ url: props.extInfo.optionsUrl })
}
</script>
